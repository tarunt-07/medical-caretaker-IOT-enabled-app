...
function Logs() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!note.trim()) return;
    setSubmitting(true);
    const newLog = { id: Date.now(), date: new Date().toISOString().split("T")[0], note, by: user.name || "Caretaker" };
    try { await api.addLog({ note }); } catch (err) { console.warn("Backend offline:", err.message); }
  ...
  return (
    <div className="dashboard-layout medical-bg">
      ...
      <main className="main-panel">
        ...
        <div style={{ display: "grid", gridTemplateColumns: isCaretaker ? "1fr 1.5fr" : "1fr", gap: "20px" }}>
          {isCaretaker && (
            <div className="glass-card section-card animate-in">
              <div className="section-header"><h3><div className="section-icon blue">📝</div> Add Today's Log</h3></div>
              <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
                <textarea placeholder="Describe patient condition today..." value={note} onChange={e => setNote(e.target.value)} required rows={6}
                  style={{ width: "100%", padding: "14px", borderRadius: "var(--radius)", border: "1.5px solid var(--border)", outline: "none", background: "var(--white)", color: "var(--text)", resize: "vertical", fontSize: "0.92rem" }} />
          ...
          <div className="glass-card section-card animate-in">
            <div className="section-header"><h3><div className="section-icon teal">📓</div> Log History</h3></div>
                ...
                <div key={log.id} className="log-entry" style={{ position: "relative" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    ...
                    {(isCaretaker || user.role === "doctor") && (
                      <button onClick={() => setConfirmDelete(log)}
                        style={{ padding: "4px 8px", borderRadius: "var(--radius-sm)", border: "none", background: "#fee2e2", color: "var(--danger)", fontSize: "0.75rem", cursor: "pointer", flexShrink: 0, marginLeft: "10px" }}>
                        🗑️
                      </button>
                    )}
                  </div>
